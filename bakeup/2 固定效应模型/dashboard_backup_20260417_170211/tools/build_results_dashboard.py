...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} html{scroll-behavior:smooth}
    ...
    #homeRankingSection{grid-column:1 / span 7}
    #homeDetailSection{grid-column:8 / span 5;position:sticky;top:14px;align-self:start}
    ...
    .metric .k{font-size:12px;color:var(--muted);text-transform:uppercase;letter-spacing:.04em;margin-bottom:8px}.metric .v{font-size:24px;font-weight:800;line-height:1.1}
    ...
    @media (max-width:1180px){.hero-grid,.layout,.score-strip,.detail-grid{grid-template-columns:1fr}.sidebar{position:static;grid-template-columns:1fr}.metric-grid{grid-template-columns:repeat(2,1fr)}#homeRankingSection,#homeDetailSection{grid-column:1 / -1;position:static}}
...
<body>
    ...
    <div class="layout">
      ...
      <main class="content">
        <section class="card" id="homeRankingSection">
          ...
          <div class="table-wrap"><table class="ranking" id="rankingTable"></table></div>
        </section>
        ...
        <section class="card hidden" id="matrixSection">
          ...
          <div class="table-wrap"><table class="matrix" id="matrixTable"></table></div>
        </section>
      </main>
    </div>
  </div>
  ...
  <script>
    ...
    function renderDetail(model) {
      ...
      const vif = (data.vif_tables[model.model_id] || []).slice(0,12);
    ...
    function renderAll(refilter=true) {
      ...
      if (pageKind === 'home') {
        renderRanking(list);
        renderDetail(list.find(x => x.model_id === state.selected) || null);
