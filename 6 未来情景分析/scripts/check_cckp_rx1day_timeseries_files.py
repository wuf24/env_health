...
from config_cckp_rx1day_timeseries import LOG_DIR


def main() -> int:
    ensure_runtime_directories()
    logger, log_path = configure_logger("check_cckp_rx1day_timeseries_files")
    logger.info("Scanning directory: %s", OUTDIR)
    logger.info("Log file: %s", log_path)
    for note in validation_notes():
        logger.info("%s", note)

    found: dict[tuple[str, str], list[dict[str, object]]] = defaultdict(list)
    ...
    for path in iter_download_files(OUTDIR):
        ...
        key = combination_key(metadata["scenario"], metadata["statistic"])
        found[key].append(
            {
                "file_name": path.name,
    ...
    for combo in expected_combinations():
        key = combination_key(combo["scenario"], combo["statistic"])
        files = found.get(key, [])
        valid_files = [item for item in files if item["valid"]]

        if not files:
            status = "missing"
            missing.append(build_output_stem(combo["scenario"], combo["statistic"], area_code=AREA_CODE))
        elif len(files) > 1:
            status = "duplicate"
    ...
    status_df = pd.DataFrame(rows).sort_values(["scenario", "statistic"]).reset_index(drop=True)
    summary_path = LOG_DIR / "cckp_rx1day_timeseries_download_status.csv"
