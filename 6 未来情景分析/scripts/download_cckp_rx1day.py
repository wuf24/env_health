...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download CCKP China provincial rx1day files in batch.")
...
def process_combination(
    ...
    period = combo["period"]
    combo_label = f"{scenario} | {percentile} | {period}"
    existing_files = find_matching_files(scenario, percentile, period, outdir=OUTDIR)
    ...
    for output_format in preferred_formats(args.output_format):
        url = build_url(
            ...
            percentile_token=PERCENTILE_TOKEN,
...
def main() -> int:
    args = parse_args()
    ensure_runtime_directories()
    logger, log_path = configure_logger("download_cckp_rx1day")
